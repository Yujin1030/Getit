	...
	<script type="text/javascript">
		...
		webSocket.onmessage = function(message) {
			...
			if(node.status === "visit") {
				...
				// 아래에 메시지를 추가한다.
				$div.find("#textArea").append('<div style="width:100%;height:8%;text-align:left;" id="inputText"><input type="button" class="btn btn-dark" value="'+node.message+'"></div>');
		...
		$(document).on("click", "#sendBtn", function(){
			...
			// 아래에 메시지를 추가한다.
			$div.find("#textArea").append('<div style="width:100%;height:8%;text-align:right;" id="inputText"><input type="button" class="btn btn-secondary" value="'+message+'"></div>');
			$div.find("#textArea").scrollTop($div.find("#textArea")[0].scrollHeight);
